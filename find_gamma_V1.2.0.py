...
assert gyroSensor.connected, "Error: gyroscope not connected"
gyroSensor.mode = "GYRO-RATE" # Calibrate gyro
gyroSensor.mode = "GYRO-ANG"
sleep(1)
assert gyroSensor.value() == 0, "Error: gyroscope is uncalibrated"
...
leftMotor.reset()
ultraMotor.reset()
rightMotor.stop_action = "hold"
leftMotor.stop_action = "hold"
...
def claw(command):
    ...
    if (command == "open"):
        print("Opening Claw")
        clawMotor.run_timed(time_sp = 1300, speed_sp = -1000)
    if (command == "close"):
        print("Closing claw")
        clawMotor.run_timed(time_sp = 1700, speed_sp = 1000, stop_action="hold")
    if (command == "stop"):
        print("Stopping claw")
        clawMotor.stop()
        return
...
def gyroCalibrate():
    ...
    rightMotor.run_to_abs_pos(speed_sp = TURN_SPEED)
    leftMotor.run_to_abs_pos(speed_sp = TURN_SPEED)
