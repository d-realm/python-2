...
def scan():
    ...
    # Set position to -95, this turns it 90 degrees clockwise
...
def ultraCalibrate(direction):
    ...
    while (ultraSensor.value() >= right - U_OFFSET or ultraSensor.value() <= right + U_OFFSET ):
        rightMotor.run_forever(speed_sp = -direction * TURN_SPEED)
        leftMotor.run_forever(speed_sp = direction * TURN_SPEED)
